#!/usr/bin/env python3
"""
Verification script for S04 (template remediation wave 3 — batches 50-66).
Validates batch3-report.json, README quality, compose healthchecks,
restart policies, and description cleanliness for 162 remediated templates.

Run: python scripts/verify-s04.py
"""
import json
import os
import re
...
def main():
    print("=" * 60)
    print("S04 Verification Script (Batches 50-66 Remediation)")
    print("=" * 60)

    templates_dir = os.path.join(ROOT, "templates")

    # ---------------------------------------------------------------
    # 1. batch3-report.json exists and has correct summary
    # ---------------------------------------------------------------
    report_path = os.path.join(ROOT, ".gsd", "milestones", "M008", "slices", "S04", "batch3-report.json")
    report_exists = os.path.isfile(report_path)
    check("batch3-report.json exists", report_exists)

    if not report_exists:
        print(f"\n{FAIL} — Cannot continue without batch3-report.json")
        return 1
    ...
    check("batch3-report shows >= 162 remediated", remediated >= 162,
          f"remediated={remediated}")
    check("batch3-report shows 0 failures", failed == 0,
          f"failed={failed}")

    # ---------------------------------------------------------------
    # 2. All 162 template IDs in report exist on disk
    # ---------------------------------------------------------------
    templates_list = report.get("per_template", [])
    report_ids = [t["id"] for t in templates_list]
    check("batch3-report has 162 entries", len(report_ids) == 162,
    ...
    # ---------------------------------------------------------------
    # 6. Compose healthcheck: all services should have healthcheck
    # ---------------------------------------------------------------
    ...
    for tid in report_ids:
        ...
        try:
            ...
            if "healthcheck:" not in content:
                healthcheck_failures.append(tid)
        except Exception:
            pass

    check("All compose files have healthcheck", len(healthcheck_failures) == 0,
          f"{len(healthcheck_failures)} missing" if healthcheck_failures else "all have healthcheck")

    # ---------------------------------------------------------------
    # 7. Wave results files exist and sum correctly
    # ---------------------------------------------------------------
    wave_dir = os.path.join(ROOT, ".gsd", "milestones", "M008", "slices", "S04")
    wave_total = 0
    wave_ok = True
    for i in [8, 9, 10]:
        ...
        if wave_failed > 0:
            wave_ok = False
    check("Wave results sum to >= 162 successes", wave_total >= 162,
          f"total={wave_total}")
    check("All waves have 0 failures", wave_ok)

    # ---------------------------------------------------------------
    # 8. Spot-check 5 random templates for deep quality
    # ---------------------------------------------------------------
    random.seed(42)
    ...
    for tid in spot_targets:
        ...
        # Check compose has healthcheck and restart
        ...
        if os.path.isfile(compose_path):
            ...
            if "restart:" not in cc:
                issues.append("Compose missing restart policy")
    ...
    # ---------------------------------------------------------------
    # 9. Run test-audit-templates.py regression suite
    # ---------------------------------------------------------------
    import subprocess
    ...
    print("SUMMARY")
    print("=" * 60)
    print(f"  Batch 3 report:       {remediated} remediated, {failed} failed")
    ...
    print(f"  Healthchecks:         {len(healthcheck_failures)} missing")
