#!/usr/bin/env python3
"""
process-yunohost.py - Direct template creation from fact-cards.json
...
Usage:
    python3 scripts/process-yunohost.py [--dry-run]
"""
...
FACT_CARDS_PATH = os.path.join(ROOT_DIR, 'fact-cards.json')
TEMPLATES_DIR = os.path.join(ROOT_DIR, 'templates')
REVIEW_QUEUE_PATH = os.path.join(ROOT_DIR, 'review-queue.json')
...
def extract_tags(candidate_name, hints):
    ...
    if 'yunohost-source' in hints: tags.append('yunohost')
    return tags[:4]


def create_arcane_json(slug, data):
    """Build arcane.json per template-builder agent contract."""
    name = data.get('candidate', slug)
    tags = extract_tags(name, data.get('classification_hints', []))
    title = name.replace('-', ' ').replace('_', ' ').title()
    return {
        "id": slug,
        "name": title,
        "description": "Self-hosted %s deployment via Docker, sourced from YunoHost catalog" % title,
        "version": "1.0.0",
        "author": "Arcane",
        "tags": tags
    }
...
def create_readme(slug, data):
    ...
    lines.append("")
    if github_url:
        lines.append("[%s](%s) is a self-hosted application available through the YunoHost catalog." % (name, github_url))
    else:
        lines.append("%s is a self-hosted application available through the YunoHost catalog." % name)
    lines.append("")
    ...
    lines.append("## Source")
    lines.append("")
    lines.append("- YunoHost catalog entry: `%s`" % name)
    if github_url:
...
def create_template(slug, data, dry_run=False):
    ...
    # arcane.json
    arcane = create_arcane_json(slug, data)
    ...
    # README.md
    readme = create_readme(slug, data)
...
def build_review_entry(candidate):
    ...
    return {
        "candidate": name,
        "source": candidate.get('source', 'yunohost'),
...
def main():
    ...
    print("Loaded %d fact cards" % len(fact_cards))
    ...
    for candidate in reachable:
        ...
        slug = slugify(name)
        if create_template(slug, candidate, dry_run):
            created += 1
        else:
            skipped += 1
    ...
    for candidate in unreachable:
        review_entries.append(build_review_entry(candidate))
    
    if not dry_run:
        with open(REVIEW_QUEUE_PATH, 'w', encoding='utf-8') as f:
            json.dump(review_entries, f, indent=2)
            f.write('\n')
        print("Wrote %d entries to review-queue.json" % len(review_entries))
    else:
        print("[DRY-RUN] Would write %d entries to review-queue.json" % len(review_entries))
