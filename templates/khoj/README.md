# Khoj — AI Personal Search Assistant

[Khoj](https://khoj.dev) is an open-source, AI-powered search and chat assistant for your personal knowledge base. Index your documents, notes, code, and markdown files, then search or chat with them using natural language. Khoj supports both cloud LLMs (OpenAI) and local models (Ollama, LM Studio) for fully offline operation.
...
**Index content from the command line:**

```bash
# Index a markdown file
curl -X POST http://localhost:42110/api/config/content \
  -H "Content-Type: application/json" \
  -d '{"input_files": ["/path/to/notes.md"]}'
```

**Search from the command line:**

```bash
curl -X POST http://localhost:42110/api/search \
  -H "Content-Type: application/json" \
  -d '{"query": "Docker deployment notes"}'
```

## Using Local LLMs with Khoj

Khoj works with Ollama, LM Studio, and any OpenAI-compatible API endpoint:

1. **Configure a local LLM in `.env`:**

   ```
   KHOJ_LLM_MODEL_NAME=llama3.1:8b
   KHOJ_LLM_API_KEY=anything
   KHOJ_LLM_API_BASE_URL=http://host.docker.internal:11434/v1
   ```

2. **Leave `OPENAI_API_KEY` empty.**
...
The model downloads on first use. Subsequent chat requests route through the local endpoint.
