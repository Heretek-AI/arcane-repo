...
[KAG](https://github.com/OpenSPG/KAG) (Knowledge-Augmented Generation) by OpenSPG is a framework that integrates knowledge graphs with LLMs to produce more accurate, explainable, and grounded AI responses. Unlike traditional RAG that relies on flat vector similarity, KAG structures knowledge as entity-relationship graphs for richer reasoning.

> **Note:** This template builds KAG from source using a custom Dockerfile. The first build may take several minutes.
...
```bash
docker compose logs -f kag
```

**Build multiple knowledge bases:**

```bash
# Build from different source texts for different domains
curl -X POST http://localhost:8000/build \
  -H "Content-Type: application/json" \
  -d '{"source": "Paris is the capital of France.", "knowledge_base": "geography"}'
```

**Restart after configuration changes:**

```bash
docker compose restart kag
```
